...
# 85
unsigned char password[4] = {0xFF, 0xFF, 0xFF, 0xFF};
unsigned char user_input[4];
volatile unsigned char key_ready = 0;
volatile unsigned char display_digits[4] = {0xFF, 0xFF, 0xFF, 0xFF};
volatile unsigned int hold_counter = 0 ;
volatile unsigned char hold_flag2 = 0;
volatile unsigned char key_pressed = 0xFF;
volatile unsigned char current_key_pressed = 0xFF;
...
if (digit < 20) {
segment_code = segment_map[digit];
} else {
segment_code = 0b01000000;
}
PORTC = segment_code;
PORTA = (1 << position);
}
...
static unsigned char last_key_pressed = 0xFF;
static unsigned int debounce_counter = 0;
static unsigned char debounce_state = 0;
...
if (!(PORTB & (1 << (col + 4)))) {
unsigned char current_key = key_map[row][col];
...
key_ready = 1;
debounce_state = 1;
} else if (debounce_state == 1) {
debounce_counter++;
if (debounce_counter >= 20) {
current_key_pressed = current_key;
debounce_state = 2;
}
}
}
...
user_input[0] = 0b01000000;
display_digits[0] = 0b01000000;
...
const unsigned char states[] = {0b0001, 0b0010, 0b0100, 0b1000};
static int i = 0;
while (steps != 0) {
...
const unsigned int hold_threshold = 100;
unsigned char hold_flag1 = 0;
...
hold_counter = 0;
while (hold_flag2 == 3 && hold_counter < 6000) {
...
if (senha_valida) {
stepper_set(-4,500);
unsigned char Open[4] = {14, 10, 18, 0};
display_message(Open, 2000);
clear_display();
...
unsigned char error[4] = {16, 11, 11, 10};
display_message(error, 2000);
clear_display();
_delay((unsigned long)((100)*(20000000/4000.0)));
...
while(!key_ready && hold_counter < 6000){
unsigned char inicio[4] = {0, 0, 0, 0};
display_message(inicio,10);
...
stepper_set(4,500);
hold_flag2 = 3;
unsigned char cLSE[4] = {10, 5 , 12, 13};
display_message(cLSE, 200);
clear_display();
...
unsigned char erro[4] = {16, 11, 11, 10};
display_message(erro, 2000);
clear_display();
